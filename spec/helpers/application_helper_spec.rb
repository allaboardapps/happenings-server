...
describe ApplicationHelper, type: :helper do
  describe "#humanize_uuid" do
    xit "formats a uuid string to a default number of characters with elipsis" do
      user = FactoryGirl.create :user
      expect(humanize_uuid(user.id).length).to eq 13
      expect(humanize_uuid(user.id)).to eq "#{user.id[0..9]}..."
    end

    xit "formats a uuid string to a specified number of characters with elipsis" do
      user = FactoryGirl.create :user
      expect(humanize_uuid(user.id, 5).length).to eq 8
      expect(humanize_uuid(user.id, 5)).to eq "#{user.id[0..4]}..."
    end

    xit "formats a uuid string to a specified number of characters without elipsis" do
      user = FactoryGirl.create :user
